...
The model is `claude-sonnet-4-6`, prompt version `v11`, up to 2 attempts (second attempt uses temperature 0 and 4500 max tokens). Opus 4.7 is supported as override but doesn't accept the `temperature` param. MAE values stored in `formula_config` reflect calibration runs against the current model+prompt; the hardcoded fallbacks in `calibration.ts` (1.27/0.92) are historical defaults from the original spreadsheet — not authoritative.
