 ...
 * saída com ela (`piloto-<versão>-<ts>.json`). Chamava-se `pilot-prompt-v25.ts` enquanto a
 * versão viva já era outra, então rodava a v26/v27 sob um nome dizendo v25 — a mesma
 * rotulagem mentirosa que a trava de `PROMPT_VERSION` existe pra impedir dentro do banco.
 *
 * 🔴 RODAR NÃO É MEDIR: o resultado se JULGA com `npm run consistency -- --piloto=<o json>`
 * (seção 5, pareada), e julgar é **US$0**. O `--baseline` NÃO enxerga este piloto — ele não
 * grava em `category_scores` nem em `ai_evaluation_scores`, que é o que as dimensões 1–3 leem.
 * ⚠️ Isto não é conselho de estilo: o piloto da v27 (US$1,20) ficou dias sem julgamento
 * formal enquanto a branch que o exigia seguia dizendo "falta o piloto". Julgue na mesma
 * sessão em que gastar, e liste `.pilot/` antes de rodar — o resultado pode já existir.
 *
 * Uso:
