...
function bestRecordForTitle(records: Record<string, unknown>[], query: string) {
 ...
 */
export async function fetchJikanMangaByTitle(title: string): Promise<JikanMangaDetail | null> {
  try {
    const url = new URL(`${JIKAN_BASE}/manga`)
    url.searchParams.set("q", title)
    ...
    const json = await res.json()
    const data: unknown[] = Array.isArray(json?.data) ? json.data : []
    const record = bestRecordForTitle(data as Record<string, unknown>[], title)
