...
/**
 * O contexto que o Next entrega ao `onRequestError`, derivado da API PÚBLICA.
 *
 * ⚠️ Era `import type { RequestErrorContext } from "next/dist/server/instrumentation/types"`
 * — caminho INTERNO, que o Next pode mover num patch sem aviso e sem quebrar o build de
 * quem não usa tipos. Derivar do `Instrumentation` exportado amarra no contrato público:
 * se a assinatura do hook mudar, o `tsc` acusa aqui.
 */
type RequestErrorContext = NonNullable<Parameters<Instrumentation.onRequestError>[2]>
