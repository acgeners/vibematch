...
- As "Reviews externas" são auxiliares e foram buscadas por similaridade de título — podem ser de uma obra DIFERENTE com nome parecido. Antes de usar uma review, verifique se ela descreve eventos compatíveis com a sinopse. Se houver conflito (personagens, gênero, premissa), IGNORE a review.
- Quando houver reviews compatíveis, use-as sempre como evidência auxiliar na avaliação das notas. Elas são especialmente úteis para tom, ritmo, romance, dinâmica do casal, drama, tragédia, humor e conteúdo adulto.
- Nas justificativas, cite reviews externas quando elas acrescentarem evidência relevante; não cite reviews quando elas forem genéricas, incompatíveis ou não ajudarem naquele critério.
...
FORMATO DA RESPOSTA:
{
  ...
  "scores": [
    {"criterion": "romance", "score": 0.0, "justification": "Faixa X-Y (Nome): motivo breve baseado em evidência"},
...
function buildUserPrompt(req: AiEvaluationRequest): string {
  ...
  if (req.sourcedReviews?.length) {
    lines.push(
      `\nReviews externas (buscadas por similaridade de título — VERIFIQUE se descrevem a mesma obra antes de usar):`
    )
    req.sourcedReviews.forEach((r, i) => {
      const matchPct = Math.round(r.matchScore * 100)
      lines.push(
        `[${i + 1}] (fonte: ${r.source}, match com o título: ${matchPct}%, título-fonte: "${r.sourceTitle}")\n${r.text}`
      )
    })
    lines.push(
      `\nLembrete: se uma review acima descrever uma obra DIFERENTE da sinopse fornecida, IGNORE-a completamente. Não use seu conteúdo como evidência.`
    )
    lines.push(
      `Instrução obrigatória: para cada nota, considere essas reviews compatíveis junto com sinopse/tags/gêneros. Quando uma review influenciar a nota ou confirmar a evidência, mencione "review externa" ou a fonte na justificativa.`
    )
...
function enforceR19AdultContentRule(
  ...
  return {
    ...
    rawResponse: {
      ...(typeof response.rawResponse === "object" && response.rawResponse !== null
        ? response.rawResponse as Record<string, unknown>
...
export async function requestAiEvaluation(
  ...
  if (!jsonMatch) {
    ...
    throw new Error("Resposta inesperada da IA. Nenhuma avaliação foi salva.")
  }

  try {
    return postProcessEvaluation(parseClaudeResponse(jsonMatch[0], req.title), req)
  } catch (err) {
