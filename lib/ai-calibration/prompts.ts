...
5. O \`user_score\` é a nota de GOSTO de quem leu (média de eixos de preferência), não uma medida de atributo. Ele é CONTEXTO — nunca justificativa. "user_score alto" não sustenta subir critério nenhum: pessoas dão nota alta a obras com pouco romance, pouca ação ou pouco drama. Se a sua justificativa só funciona citando o user_score, não emita a sugestão.
5a. Os \`post_*\` são o oposto disso e são a sua melhor evidência: são a avaliação de quem LEU a obra, dimensão a dimensão, feita depois da leitura. Eles observam o que a sinopse e as tags só sugerem. Use-os de forma dirigida — post_pacing fala de ritmo, post_impact_immersion de peso emocional, post_fl/post_ml dos protagonistas — e diga na justificativa qual deles sustenta o ajuste. ⚠️ Ele diz o quanto a pessoa GOSTOU, não a intensidade de um atributo — não use um user_score alto para subir um critério que a evidência textual não sustenta.
