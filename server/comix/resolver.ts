...
// Implementação do resolvedor da Comix. NÃO é `"use server"` de propósito: num
// arquivo com essa diretiva, TODA função exportada vira um endpoint HTTP público
// (o Next gera um id de action e qualquer um pode fazer POST nela) — e as funções
// de resolução aqui rodam em background/cascata, onde `ensureAdmin()` não serve de
// gate (sem request, `cookies()` lança → ele retornaria false e mataria as cascatas
// do próprio dono). Tirando-as do `"use server"`, elas deixam de existir como
// endpoint. A fachada `server/actions/comix-resolver.ts` reexporta como action só
// o que a UI de fato chama — e essas, sim, levam gate.
