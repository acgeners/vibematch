...
/**
 * Leitura UNCACHED do catálogo de tags (lógica única de consulta). Usada
 * diretamente em ambiente headless/CLI (sem request scope do Next, onde
 * `unstable_cache` lança "incrementalCache missing"). O runtime do Next usa o
 * wrapper cacheado `getAllTags` abaixo — mesma lógica, mesmos resultados.
 */
export async function getAllTagsUncached(): Promise<TagOption[]> {
...
/** Wrapper CACHEADO (runtime Next). Mesma lógica de `getAllTagsUncached`. */
export const getAllTags = unstable_cache(getAllTagsUncached, ["all-tags-v3"], {
  revalidate: 300,
  tags: ["tags-catalog"],
})
