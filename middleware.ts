 ...
 * `/ai-usage`, `/admin`) enumeradas à mão aqui **e** um `layout.tsx` de 3 linhas em
 * cada uma: rota nova da console entrava sem gate e sem sidebar se alguém esquecesse
 * de tocar nos dois lugares — e nada acusaria, porque a página renderiza igual. Com
 * `app/curation/*`, quem gateia é o prefixo e quem desenha é o layout do pai.
 */
const CONSOLE_PREFIXES = ["/curation"]
 ...
 *
 * 🔴 `/account` entrou aqui em 2026-08-09 depois de medido: sem sessão,
 * `getCurrentUserId()` cai no singleton (o DONO), e `/account/taste-profile` renderizava o
 ...
 * gate é aqui — e vale para `/account` inteira, não só `/perfil`, porque a aba Conta
 * mostra e-mail e papel.
 *
 * 🔴 `/dashboard` entrou no mesmo dia, pelo mesmo motivo, medido: a página imprimia o
 ...
 *
 * `/discover` ("Mais como estas") entra pela mesma régua: metade do resultado é o
 ...
 * stream já ter começado. Medido em dev — `GET /curation/settings` anônimo devolvia **200** com o
 ...
 *
 * ⚠️ Isto gateia a console (papel de curador) e `/account` (só exige sessão).
 * `/import`, `/dashboard` e o resto da matriz de acesso continuam abertos a qualquer
...
export async function middleware(request: NextRequest) {
  ...
  // Sessão é tudo que `/account` pede — o papel só importa na console.
