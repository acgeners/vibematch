...
import {
  ...
  normalizeOpenId,
  SETTINGS_GROUPS,
} from "@/app/curation/settings/sections"
import type { SettingsSection } from "@/app/curation/settings/sections"
        ...
        {group.sections.map((section) => {
          ...
          const serverCollapse = usesServerCollapse
            ? {
                open: isOpen,
                href: isOpen
                  ? `/curation/settings?g=${group.id}#card-${section.id}`
                  : `/curation/settings?g=${group.id}&open=${section.id}#card-${section.id}`,
              }
            : undefined
...
async function ItemBody({
  ...
  switch (section.id) {
      ...
      return (
        <div className="space-y-4">
          <ConsoleSectionNote accent="violet">
            ...
            <Link
              href="/curation/settings?g=ia#card-embeddings"
          ...
          <CalibrationPanel
            ...
            accent="violet"
            aiPending={[
              { label: "Embeddings", count: embeddings, href: "/curation/settings?g=ia#card-embeddings" },
              {
                label: "Sinopse canônica",
                count: canonicalSynopsis,
                href: "/curation/settings?g=ia#card-synopsis-canonical",
              },
              {
                label: "Resumo de reviews",
                count: reviewSummary,
                href: "/curation/settings?g=ia#card-review-synthesis",
              },
            ]}
          />
        </div>
    ...
    case "review-synthesis": {
      ...
      // /curation/works. Não foi arrumação: o painel era cego (sabia dizer "125
        ...
        <div className="space-y-4">
          ...
          {open && (
            <div className="border-t border-border/60 pt-4">
              <Suspense fallback={<BodySkeleton />}>
                <TagConsolidationTool params={tagParams} basePath="/curation/settings" />
              </Suspense>
            </div>
          )}
        </div>
...
function ExpandControl({
  ...
  const href = open
    ? `/curation/settings?g=${groupId}#card-${sectionId}`
    : `/curation/settings?g=${groupId}&open=${sectionId}#card-${sectionId}`
  return (
    <Link
      href={href}
