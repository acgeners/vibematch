 ...
 * /curation/works virar duas páginas — a outra metade (IA Atributos, Tags &
 * Reviews) mora em app/curation/works/page.tsx. Mesma tag de invalidação
...
/**
 * "Suas notas de IA" (`/my-ai-scores`) — antiga "Fila de Recomendação", renomeada em
 * 2026-08-16 porque o nome velho descrevia errado nos dois pedaços:
 *
 * - **"recomendação" não aparecia no conteúdo.** As abas produzem **Veredito IA** e
 *   **Interesse** — notas POR OBRA. E existe uma `/recommendations` de verdade (o consultor
 *   + histórico de rodadas): duas rotas dividiam a palavra, e quem a levava no nome era
 *   justamente a que não a produz.
 * - **"fila" promete que algo anda sozinho.** Aqui nada anda: você filtra, seleciona e
 *   dispara ações PAGAS. É lista de trabalho, não fila.
 *
 * O nome novo recorta o que de fato mora aqui — as duas notas de IA que são **suas** — e
 * separa das vizinhas: `/curation/works` são as notas da OBRA (catálogo, curador) e o
 * `/ranking` mostra a Nota Prevista. A descrição do `Header` abaixo já dizia isso desde
...
export default async function FilaRecomendacaoPage({
  ...
  const rkHref = `/my-ai-scores?${rkParams}`
  ...
  const synHref = `/my-ai-scores?${synParams}`
  ...
  const untrackedHref = `/my-ai-scores?${untrackedParams}`
