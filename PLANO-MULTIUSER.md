...
- **PENDENTE do usuário (Fase 1b):** (1) aplicar migration 137 no SQL editor; (2) no painel Supabase → Auth: habilitar provider email/senha e decidir confirmação de email (on = signup manda link; off = entra direto).
- **Próximo: Fase 2** — inclui o re-chaveamento deliberado (dono "reivindica" a linha singleton + dados per-user pro uid de auth) + a partição `works`→`user_work_state` + `calculated_scores`/`taste_profile`/`formula_config` per-user. É aqui que os getters/setters passam a resolver por usuário.
