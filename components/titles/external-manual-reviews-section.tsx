...
export function ExternalManualReviewsSection({ workId, reviews }: Props) {
  ...
  const form = useForm<FormFields>({
    resolver: zodResolver(externalReviewInputSchema) as unknown as Resolver<FormFields>,
    defaultValues: EMPTY,
    mode: "onSubmit",
  })

  const beginEdit = (r: ExternalManualReviewDisplayRow) => {
    setEditingId(r.id)
  ...
  const onSubmit = (values: FormFields) => {
    startTransition(async () => {
      const result = editingId
        ? await updateExternalManualReview(editingId, workId, values)
        : await createExternalManualReview(workId, values)
      if (result.ok) {
        toast.success(editingId ? "Review externa atualizada." : "Review externa adicionada.")
        setEditingId(null)
        form.reset(EMPTY)
        refresh()
      } else {
      ...
      <div
        onKeyDown={(e) => {
          if (e.key === "Enter" && (e.target as HTMLElement).tagName !== "TEXTAREA") {
            e.preventDefault()
            e.stopPropagation()
            if (!pending) form.handleSubmit(onSubmit)()
          }
        }}
      ...
      >
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold text-muted-foreground">
            {editingId ? "Editar review externa" : "Adicionar review externa"}
          </span>
          {editingId && (
        ...
        <Button type="button" size="sm" disabled={pending} onClick={() => form.handleSubmit(onSubmit)()}>
          {editingId ? <PencilLine className="mr-1 h-3.5 w-3.5" /> : <Plus className="mr-1 h-3.5 w-3.5" />}
          {editingId ? "Salvar alterações" : "Adicionar review externa"}
        </Button>
      </div>
          ...
          {reviews.map((r) => (
            <li key={r.id} className="rounded-md border bg-card/40 p-3">
              <div className="flex flex-wrap items-baseline justify-between gap-2 text-xs">
                <span className="font-semibold">{PLATFORM_LABELS[r.source] ?? r.source}</span>
                <div className="flex items-center gap-1">
                  <Button type="button" variant="ghost" size="sm" className="h-7 px-2" disabled={pending} onClick={() => beginEdit(r)}>
