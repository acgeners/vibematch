...
const READING_STATUSES_WITH_PROGRESS = new Set(["Lendo", "Finalizado", "Retomar", "Retomar (tenso)", "Droppado", "Pausado", "Esp. Temp"])
...
const POST_READING_SCORE_FIELDS = [
  ...
  {
    name: "post_protagonists_score",
    label: "Protagonistas",
    help: (
      <div className="space-y-3">
        <ul className="list-disc space-y-1 pl-4">
          <li>Carisma / presença</li>
          <li>Inteligência / agência (tomam decisões relevantes?)</li>
          <li>Consistência de comportamento</li>
        </ul>
        <p>👉 O protagonista carrega a obra ou é arrastado por ela?</p>
      </div>
    ),
  },
  {
...
export function WorkForm({ workId, initialValues }: WorkFormProps) {
  ...
  const personalStatus = watch("personal_status")
  const coverUrl = watch("cover_url")
  const manualScore = watch("manual_score")
  const titleValue = watch("title")
  const yearValue = watch("year")
  const yearEndValue = watch("year_end")
  ...
  const [postReadingWeights, setPostReadingWeights] = useState<Record<PostReadingScoreField, number>>({
    ...DEFAULT_POST_READING_WEIGHTS,
  })
  ...
  useEffect(() => {
    ...
    try {
      ...
      setBatchDrafts(Array.isArray(parsed) ? parsed : [])
    } catch {
      setBatchDrafts([])
    }
  }, [isCreating])
  ...
  useEffect(() => {
    ...
    try {
      ...
      setPostReadingWeights({ ...DEFAULT_POST_READING_WEIGHTS })
    }
  }, [])
  ...
  useEffect(() => {
    const weightedScores = postReadingScores.reduce(
      (acc, score, index) => {
        if (typeof score !== "number" || !Number.isFinite(score)) return acc
        const weight = postReadingWeights[POST_READING_SCORE_FIELDS[index].name]
        return {
          scoreSum: acc.scoreSum + score * weight,
  ...
  const handleExternalSelect = (data: ExternalWorkData) => {
    ...
    if (data.genres?.length) setValue("genres", data.genres)
  ...
  const onSubmitAddMore = async (values: WorkFormValues) => {
    if (workId) return
    scrollToTop()
  ...
  const handleCreateBatch = async () => {
    ...
    try {
      ...
      let valuesToCreate = batchDrafts.map((draft) => draft.values)
      ...
      if (currentTitle) {
        ...
        setTopFeedback("Verificando duplicidade...")
        if (await checkDuplicateBeforeCreate(parsedCurrent.data, "addMore")) return
        setTopFeedback("Criando lote e recalculando notas...")
        valuesToCreate = [...valuesToCreate, parsedCurrent.data]
      }
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <ChipInput
                    value={watch("alternative_titles") ?? []}
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-[minmax(0,220px)_minmax(0,160px)_minmax(0,180px)]">
                  <div className="space-y-1.5">
                    ...
                    <Select
                      value={watch("publication_status")}
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="year_range">
                      {pubStatus === "C" ? "Ano de início/fim" : "Ano de início"}
                    </Label>
                    <Input
                      id="year_range"
                      inputMode="numeric"
                      placeholder={pubStatus === "C" ? "Ex: 2020 - 2024" : "Ex: 2022"}
                      value={formatYearRange(yearValue, yearEndValue, pubStatus === "C")}
                      onChange={(event) => {
                        const { start, end } = parseYearRange(event.target.value)
                        setValue("year", start, { shouldDirty: true, shouldValidate: true })
                        setValue("year_end", pubStatus === "C" ? end : null, {
                          shouldDirty: true,
                          shouldValidate: true,
                        })
                      }}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,240px)_minmax(0,240px)_minmax(0,180px)]">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={watch("synopsis_quality") ?? "none"}
                    onValueChange={(v) =>
            ...
            {hasProgress && (
              <div className="space-y-4 border-t pt-6">
                ...
                <div>
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Rating 0 - 10</p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {POST_READING_SCORE_FIELDS.map((field) => (
                    <div key={field.name} className="space-y-1.5">
                      ...
                      <Input
                        ...
                        max={10}
                        placeholder="—"
                        {...register(field.name as PostReadingScoreField, {
                          setValueAs: optionalNumber,
                        })}
                      />
                    </div>
