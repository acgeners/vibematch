...
import { workFormSchema, type WorkFormValues, type WorkFormInput } from "@/lib/validations/work.schema"
...
import { listTagCatalog } from "@/server/actions/external"
import type { TagCatalogItem } from "@/server/actions/external"
...
const uniqueByTagName = (values: string[]) => {
...
export function WorkForm({ workId, initialValues }: WorkFormProps) {
  ...
  const catalogTokenIndex = useMemo(() => {
    const index = new Map<string, TagCatalogItem[]>()
    for (const tag of tagCatalog) {
      const tokens = tag.name.toLowerCase()
        .replace(/[^a-z0-9\s]/g, " ")
        .split(/\s+/)
        .filter((t) => t.length >= 4)
      for (const token of tokens) {
        if (!index.has(token)) index.set(token, [])
        index.get(token)!.push(tag)
      }
    }
    return index
  }, [tagCatalog])

  const isGenreTag = useCallback((tag: TagCatalogItem | undefined) => (
    tag?.tag_group_id === GENRE_TAG_GROUP_ID || tag?.groupSlug === "genre"
  ), [])

  const fuzzyFindInCatalog = useCallback((value: string): TagCatalogItem | undefined => {
    const exact = catalogByName.get(normalizeTagName(value))
    ...
    const candidates = new Map<TagCatalogItem, number>()
    for (const token of valueTokens) {
      for (const tag of catalogTokenIndex.get(token) ?? []) {
        candidates.set(tag, (candidates.get(tag) ?? 0) + 1)
      }
    }
    for (const [tag, count] of candidates) {
      if (count / valueTokens.size >= 0.6) return tag
    }
    return undefined
  }, [catalogByName, catalogTokenIndex])

  const filterCatalogValues = useCallback((values: string[], expected: "genre" | "tag") => uniqueByTagName(values)
    .flatMap((value) => {
      const catalogTag = fuzzyFindInCatalog(value)
      if (!catalogTag) return expected === "tag" ? [value.trim()] : []
      const isGenre = isGenreTag(catalogTag)
      return expected === "genre" === isGenre ? [catalogTag.name] : []
    }), [fuzzyFindInCatalog, isGenreTag])

  const splitCatalogValues = useCallback((genreValues: string[], tagValues: string[]) => {
    const genres: string[] = []
    const tags: string[] = []
    for (const value of uniqueByTagName(genreValues)) {
      const catalogTag = fuzzyFindInCatalog(value)
      if (catalogTag && isGenreTag(catalogTag)) {
        genres.push(catalogTag.name)
      } else if (catalogTag) {
        tags.push(catalogTag.name)
      }
    }
    for (const value of uniqueByTagName(tagValues)) {
      const catalogTag = fuzzyFindInCatalog(value)
      if (catalogTag && isGenreTag(catalogTag)) {
        genres.push(catalogTag.name)
      } else if (catalogTag) {
    ...
    return { genres: uniqueByTagName(genres), tags: uniqueByTagName(tags) }
  }, [fuzzyFindInCatalog, isGenreTag])

  const genreChipGroups = useMemo(() => {
    const genres = filterCatalogValues(genresValue ?? [], "genre")
    return genres.length ? [{ label: "Genre", values: genres }] : []
  }, [filterCatalogValues, genresValue])

  const tagChipGroups = useMemo(() => {
    const groups = new Map<string, string[]>()
    const ungrouped: string[] = []
    for (const value of tagsValue ?? []) {
      const catalogTag = fuzzyFindInCatalog(value)
      if (!catalogTag || isGenreTag(catalogTag)) {
        if (!catalogTag) ungrouped.push(value)
        continue
      }
      const label = catalogTag.groupLabel
      groups.set(label, [...(groups.get(label) ?? []), catalogTag.name])
    }
    if (ungrouped.length) groups.set("Sem grupo", ungrouped)
    return [...groups.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([label, values]) => ({ label, values: uniqueByTagName(values) }))
  }, [fuzzyFindInCatalog, isGenreTag, tagsValue])
  ...
  useEffect(() => {
    ...
    const split = splitCatalogValues(genresValue ?? [], tagsValue ?? [])
    if (JSON.stringify(split.genres) !== JSON.stringify(genresValue ?? [])) {
      setValue("genres", split.genres, { shouldDirty: false, shouldValidate: false })
    }
    if (JSON.stringify(split.tags) !== JSON.stringify(tagsValue ?? [])) {
      setValue("tags", split.tags, { shouldDirty: false, shouldValidate: false })
    }
  }, [genresValue, setValue, splitCatalogValues, tagCatalog.length, tagsValue])
  ...
  const handleExternalSelect = (data: ExternalWorkData) => {
    ...
    if (data.genres?.length || data.tags?.length) {
      const split = splitCatalogValues(data.genres ?? [], data.tags ?? [])
      setValue("genres", split.genres, { shouldDirty: true, shouldValidate: true })
      setValue("tags", split.tags, { shouldDirty: true, shouldValidate: true })
    }
        ...
        {openSections.basic && (
          <CardContent className="space-y-6">
            ...
            <div className="space-y-1.5">
              ...
              <ChipInput
                value={genresValue ?? []}
                groups={genreChipGroups}
                onChange={(v) => setValue("genres", uniqueByTagName(v), { shouldDirty: true, shouldValidate: true })}
                placeholder="Ex: Romance, Fantasy (Enter para adicionar)"
              />
              <p className="text-xs text-muted-foreground">
                Apenas tags do grupo Genre no Supabase ficam nesta lista.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <ChipInput
                value={tagsValue ?? []}
                groups={tagChipGroups}
                onChange={(v) => setValue("tags", filterCatalogValues(v, "tag"), { shouldDirty: true, shouldValidate: true })}
                placeholder="Ex: reencarnação, isekai (Enter para adicionar)"
              />
            </div>
          </CardContent>
        )}
